...
func (s *Server) handleUpload(w http.ResponseWriter, r *http.Request) {
	if r.Method != "POST" {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusMethodNotAllowed)
		json.NewEncoder(w).Encode(map[string]string{"error": "upload_requires_post_method"})
		return
	}
	...
	if err := r.ParseMultipartForm(maxUploadSize); err != nil {
		...
		json.NewEncoder(w).Encode(map[string]string{"error": "file_too_large_max_800mb"})
		return
	}
	...
	if len(key) != keyLength || !isValidKey(key) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]string{"error": "invalid_key_format"})
		return
	}
	...
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]string{"error": "no_file_uploaded"})
		return
	}
	defer file.Close()
	...
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]string{"error": "cannot_read_file"})
		return
	}
	file.Seek(0, 0)
	...
	if !isEPUBMime && !isEPUB(file) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]string{"error": "only_epub_files_allowed"})
		return
	}
	file.Seek(0, 0)
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(map[string]string{"error": "failed_to_save_file"})
		return
	}
	...
	if _, err := io.Copy(destFile, file); err != nil {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(map[string]string{"error": "failed_to_write_file_data"})
		return
	}
